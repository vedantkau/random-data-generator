...
def validate_exp(exp, column_names, current_col):
    ...
    if not check3_pass:
        return False, "Word length is too long (max 30 characters)"
...
def validate_forms(columns_details, no_of_rows):
    ...
    if no_of_rows > 250:
        return False, "Rows limit exceeded (Max 250 rows can be added)"
    if len(columns_details.keys())>15:
        return False, "Columns limit exceeded (Max 15 columns can be added)"
